...
const isEmptyString = (s) => !s || !s.replace(/\s|\n/g, '').length
...
export default class ChoicesDropdown {
  ...
  addOption (value, key) {
    ...
    // Init available options search again
    if (this.lastSearchQuery) {
      this.filterOptionsByQuery(this.lastSearchQuery)
    }
  }
  ...
  resetSearchInput () {
    if (!isEmptyString(this.lastSearchQuery)) {
      this.filterOptionsByQuery() // renders all available options
    }
  ...
  moveSelection (direction, ignoreMissing = !1) {
    const hoveredNode = this.nodes.optionsNode
      .querySelector(`[${KEY_ATTRIBUTE_NAME}="${this.hoveredKey}"]`)
    let targetNode
   ...
   * @return {Array}
   * */
  getValue = () => this.selectedOptions.map((item) => item.value)
