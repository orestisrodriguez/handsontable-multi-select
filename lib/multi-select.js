...
export class MultiSelectEditor extends TextEditor {
  ...
  prepare (row, col, prop, td, originalValue, cellProperties) {
    ...
    this.instance.addHook('afterChange', async (changesPromise) => {
      ...
      // An optimization mechanism that prevents
      // unnecessary options promise resolution.
      // NOTE: This map is updated only in case the processed cell inherits
      // editor from columnSettings.
      ...
      for await (const change of changes) {
        ...
        const selectPreferences = updatedCellProps?.select
        const selectConfig = selectPreferences?.config
        if (!selectConfig) continue
        ...
        const options = columnOptionsMap[visualColumn] ?? (
          await executePotentialFn(
            selectPreferences?.options ?? null,
            updatedCellProps,
            originalValue
          )
        )
        ...
        // Saves the options for this column if possible.
        if (columnEditor && !columnOptionsMap[visualColumn]) {
          columnOptionsMap[visualColumn] = options
        }

        // Gets all available label options
        const itemLabelKey = selectConfig?.labelKey ?? 'value'
        const availableLabels = options.map((l) => l[itemLabelKey]?.toString().trim())

        const isValid = selectConfig.isMultiple === false
          ? availableLabels.includes(updatedValue)
          : !updatedValue.split(selectConfig.separator ?? DEFAULT_SEPARATOR)
