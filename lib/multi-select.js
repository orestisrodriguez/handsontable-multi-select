...
export class MultiSelectEditor extends TextEditor {
  constructor (...props) {
    ...
    for (const [ key, value ] of Object.entries(HOOK_CALLBACKS)) {
      HOOK_CALLBACKS[key] = value.bind(this)
    }
  }
  ...
  open () {
    ...
    this.instance.addHook('beforeKeyDown', HOOK_CALLBACKS.beforeKeyDown)

    // Handles keyboard events when the search input is selected
    this.choices.addEventListener('explicitKeydown', HOOK_CALLBACKS.explicitKeydown)
  }
  ...
  close () {
    ...
    // Unsubscribe
    this.instance.removeHook('beforeKeyDown', HOOK_CALLBACKS.beforeKeyDown)
    this.choices.removeEventListener('explicitKeydown', HOOK_CALLBACKS.explicitKeydown)
  }
