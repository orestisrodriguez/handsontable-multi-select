...
export default class ChoicesDropdown {
  ...
  addValue (value, key, isCalledByUser = false) {
    ...
    this.selectedOptions.push({ key: encodeURI(key), value })
    this.nodes.valuesNode.insertBefore(this.createValueNode(value, key), this.nodes.searchNode)

    // FIX HERE: isCalledByUser, since
    // calling change may cause closing or direct
    // interaction with the input.
    // And since addValue function is called
    // to set intial state, it's
    // bad to dispatch the event right away.
    if (isCalledByUser) {
