...
export default class ChoicesDropdown {
  ...
  * */
  ...
  resizeEditor (selectedCellNode) {
    const { valuesNode, optionsNode } = this.nodes
    ...
    // Swaps dropdown's anchor to the right bound, if it
    // is outside of the screen.
    const dropdownBounds = optionsNode.getBoundingClientRect()

    const dropdownRightX = dropdownBounds.x + dropdownBounds.width
    const screenRightX = window.scrollX + window.innerWidth

    const isOutside = dropdownRightX > screenRightX

    optionsNode.style.transform = isOutside
      ? `translateX(calc(-100% + ${valuesNode.offsetWidth}px))`
      : ''
  }
