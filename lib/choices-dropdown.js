...
const SUGGESTION_HOVERED_CLASS = 'is-highlighted'
...
export default class ChoicesDropdown {
  constructor (textareaNode) {
    ...
    for (const [ key, value ] of Object.entries(NODE_CREATORS)) {
      NODE_CREATORS[key] = value.bind(this)
    }
  }
  ...
  renderStructure () {
    ...
    const searchNode = NODE_CREATORS.createSearchNode(containerNode)
    ...
    const messageNode = NODE_CREATORS.createSpinnerNode()
  ...
  resetHover () {
    ...
    if (hoveredNode) {
      removeClass(hoveredNode, SUGGESTION_HOVERED_CLASS)
    }
  }
  ...
  hoverOption (targetNode) {
    ...
    addClass(targetNode, SUGGESTION_HOVERED_CLASS)
