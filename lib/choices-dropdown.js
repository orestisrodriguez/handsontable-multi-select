...
export default class ChoicesDropdown {
  constructor (textareaNode, opts) {
    ...
    /**
     * A callback that fires on input change.
     */
   ...
   * */
  setOptions (options, itemKeys, cellArguments, optionsSettings) {
    const {
      key: itemsKey,
      value: itemsValue,
    } = itemKeys
    ...
    // Show loading message if initial
    if (initial) {
    ...
    Promise
      .resolve(fetchOptions(cellProperties, originalValue, searchTerm))
  ...
  * */
  ...
  resizeEditor (selectedCellNode) {
    const { valuesNode, optionsNode, messageNode } = this.nodes
    ...
    valuesNode.style.minHeight = `${clampedCellHeight}px`

    // Makes the selected values container have the
    // same width as the selected cell.
    const containerPaddingValue = window
      .getComputedStyle(valuesNode, null)
      .getPropertyValue('padding')
    const containerPadding = parseInt(containerPaddingValue, 10) * 2

    valuesNode.style.width = `${clampedCellWidth - containerPadding}px`
    optionsNode.style.minWidth = `${clampedCellWidth}px`

    // Swaps dropdown's anchor if it is outside the screen.
    const dropdownPosition = this.getDropdownPosition(selectedCellNode)
    optionsNode.style.transform = dropdownPosition
    messageNode.style.transform = dropdownPosition
  }
  ...
  removeAllValues () {
    ...
    const { valuesNode } = this.nodes

    // Removes all elements in the values container,
    // but the search input.
    valuesNode.childNodes.forEach(
      (childNode) => {
        if (childNode === this.nodes.searchNode) {
          return
        }

        valuesNode.removeChild(childNode)
      }
    )
  }
  ...
  /**
   * Returns true if the search input is in focus.
   *
   * @return {Boolean}
   * */
  isSearchInFocus () {
    return document.activeElement === this.nodes.searchNode
  }
  ...
  /**
   * Returns x and y position depending on space available for the dropdown to be displayed.
   *
   * @param selectedCellNode
   * @returns string
   */
  getDropdownPosition (selectedCellNode) {
    const { parentNode: textareaNode, optionsNode } = this.nodes

    const sheetRect = textareaNode.parentNode.getBoundingClientRect()
    const selectedCellRect = selectedCellNode.getBoundingClientRect()
    const optionsRect = optionsNode.getBoundingClientRect()

    const dropdownRightX = selectedCellRect.x + selectedCellRect.width
    const screenRightX = window.scrollX + window.innerWidth
    const isOutsideX = dropdownRightX > screenRightX
    const translateX = `translateX(calc(-100% + ${optionsRect.width}px))`

    const dropdownBottomY = selectedCellRect.bottom + optionsRect.height
    const multiSelectBorderWidth = getComputedStyle(document.documentElement)
      .getPropertyValue('--multi-select-choices-item-border-width')

    const isOutsideY = sheetRect.bottom < dropdownBottomY
    const translateY = `translateY(calc(-100% - ${selectedCellRect.height}px - 2 * ${multiSelectBorderWidth}))`

    if (isOutsideX && isOutsideY) return `${translateX} ${translateY}`
    if (isOutsideX) return translateX
    if (isOutsideY) return translateY
